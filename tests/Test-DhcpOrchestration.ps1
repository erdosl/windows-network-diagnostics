...
function New-SnapshotIdentity { [pscustomobject]@{ComputerName='SYNTHETIC';RunId=[guid]::NewGuid().ToString();CollectorVersion='0.4.1';IsElevated=$false;StartedAt='2026-01-01T10:00:00+00:00'} }
...
Assert-Run ($run.Evidence.CollectionStatus -eq 'Complete' -and $run.Evidence.SchemaVersion -eq 8) 'Malformed optional files do not discard completed snapshot'
