> Historical reference preserved from collector 0.6.0. For current usage see [CURRENT.md](CURRENT.md).

# Windows network diagnostics

The 0.5.0 additions are documented stage by stage in
[Evidence extensions](EVIDENCE-EXTENSIONS.md): incident/configuration inventory,
...
Read the [current reference](CURRENT.md) for parameters, output placement,
exit status, schema/contract compatibility, synthetic examples and troubleshooting.
See the [current validation matrix](VALIDATION-CURRENT.md) for tested coverage
...
See [validation results](VALIDATION.md) for the actual environment, commands,
...
completed targeted Windows 11 validation are in [WINDOWS11-FINDINGS.md](WINDOWS11-FINDINGS.md).
...
established cause. See [findings and completed validation](WINDOWS11-FINDINGS.md).
